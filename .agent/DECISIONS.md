...
- 세부(개발자 제안, CEO 확정 필요 → inbox/to-ceo.md):
  (1) 방향: 쌍 기준(A 가 B 방의 참여자여도 같은 쌍으로 봄) vs 역할 기준(A 개설·B 참여만). 제안 = **쌍 기준**.
  (2) ORPHANED 방 포함 여부: 제안 = **제외**(ACTIVE 방만 셈). B 가 옛 방을 정리하기 전에도 A 의 새 방에 들어갈 수 있게.
  (3) 오류 코드·문구: 제안 = 409 `PAIR_ROOM_EXISTS`, 카피 "걔랑은 이미 방 있잖아"(BRAND.md#5 표 추가, 디자이너 대행).
  (4) 판정 순서: 404 → 410 → 400 SELF → 이미 멤버 200 → **409 PAIR** → 409 FULL → 409 LIMIT(POST 만). 미리보기도 PAIR 까지 검사.
...
- date: 2026-09-16 (개발자 대행 기록, 사용자 결정 — 세부 4건은 제안)
