...
describe('MessageBubble (DESIGN.md#3 말풍선 3종)', () => {
  ...
  it('VOICE 입력도 마이크 아이콘 없이 시간만 (D-035 3)', () => {
    render(<MessageBubble message={userMsg(4, { inputType: 'VOICE', createdAt: '2026-09-16T12:05:00Z' })} kind="mine" showMeta />);
    expect(screen.queryByRole('img', { name: '음성' })).toBeNull();
    expect(screen.getByText('21:05')).toBeInTheDocument();
  });
});

describe('MessageBubble 듣기 버튼 없음 (D-034 8)', () => {
  it('AI 말풍선에도 "듣기" 버튼이 없다 — 재생은 보이스 모드에서만', () => {
    render(<MessageBubble message={aiMsg(3, { content: '또?' })} kind="ai" showMeta />);
