...
describe('joinErrorMessage (DESIGN.md#5 실패 표)', () => {
  it.each([
    [new ApiError(404, 'INVITE_NOT_FOUND', 'x'), '그런 방 없는데?'],
    [new ApiError(409, 'ROOM_FULL', 'x'), '여긴 벌써 꽉 찼어'],
    [new ApiError(410, 'ROOM_ORPHANED', 'x'), '방장이 도망간 방이야!'],
    [new ApiError(400, 'SELF_INVITE', 'x'), '네 방 아니야?'],
    [new ApiError(409, 'ROOM_LIMIT_EXCEEDED', 'x'), '방이 50개 넘었어. 정리하고 와!'],
    [new ApiError(409, 'PAIR_ROOM_EXISTS', 'x'), '걔랑은 이미 대화하고 있어!'], // D-022, T-023
  ])('%s → %s', (err, expected) => {
