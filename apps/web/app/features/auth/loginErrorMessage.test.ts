...
describe('loginErrorMessage', () => {
  ...
  it('access_denied → 취소 안내', () => {
    expect(loginErrorMessage('access_denied')).toBe('로그인 취소했네? 다시 시도해봐!');
  });

  it('authorization_request_not_found → 시간 초과 안내', () => {
    expect(loginErrorMessage('authorization_request_not_found')).toBe(
      '로그인 시간이 지났네? 처음부터 다시 해봐!',
    );
  });

  it('그 외 코드는 공통 실패 메시지', () => {
    expect(loginErrorMessage('oauth_failed')).toBe('로그인에 실패했네? 잠시 후 다시 시도해봐!');
    expect(loginErrorMessage('server_error')).toBe('로그인에 실패했네? 잠시 후 다시 시도해봐!');
  });

  it('배열(중복 쿼리)이면 첫 값만 본다', () => {
    expect(loginErrorMessage(['access_denied', 'oauth_failed'])).toBe('로그인 취소했네? 다시 시도해봐!');
  });
});
