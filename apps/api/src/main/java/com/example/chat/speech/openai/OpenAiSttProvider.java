...
/**
 * OpenAI `POST /audio/transcriptions` (verbose_json → text + duration 초). 호출 스레드를 블록한다.
 * 파일 포맷은 확장자로 판별되므로 SpeechService 가 content-type 기준 확장자로 저장한다.
 */
@Component
@ConditionalOnProperty(name = "app.speech.stt-provider", havingValue = "openai", matchIfMissing = true)
public class OpenAiSttProvider implements SttProvider {
	...
	public OpenAiSttProvider(WebClient openAiWebClient, SpeechProperties props) {
	...
	public Transcript transcribe(Path audio, String contentType) {
		...
		body.part("model", props.openaiSttModel());
		...
		try {
			...
			throw new SpeechException("openai stt responded " + e.getStatusCode().value(), e);
		} catch (RuntimeException e) {
			throw new SpeechException("openai stt call failed: " + rootName(e), e);
		}
		JsonNode node = parse(raw);
		JsonNode text = node.path("text");
		if (!text.isString()) throw new SpeechException("openai stt response has no text", null);
	...
	private JsonNode parse(String raw) {
		try {
			...
			throw new SpeechException("openai stt response is not json", e);
		}
	}
